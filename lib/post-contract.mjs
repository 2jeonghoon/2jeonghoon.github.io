...
export function parsePostSource({filePath, source}) {
  ...
  if (!document) fail("malformed front matter (missing delimiters)");
  let data;
  try {
    data = yaml.safeLoad(document[1]) ?? {};
  } catch (error) {
    fail(`malformed front matter (${error.message})`);
  }
  if (!data || typeof data !== "object" || Array.isArray(data)) fail("malformed front matter (expected an object)");
  const frontMatter = document[1];
  const declaredDate = frontMatter.match(/^date:\s*([^#\n]+?)\s*$/m)?.[1]
    ?.trim()
    .replace(/^(["'])(.*)\1$/, "$2");
  if (data.date instanceof Date && declaredDate) {
    const normalizedDate = data.date.toISOString().slice(0, 10);
    if (declaredDate !== normalizedDate) {
      fail("invalid date; expected a real calendar day", "date");
    }
  }
  return validatePostInput({...data, slug, body: document[2].trimEnd()});
}
